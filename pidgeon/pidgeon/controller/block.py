...
class Block(Controller):
    ...
    async def get_block_state(self, auth, user_uuid, execution_id):
        self._validate_key(user_uuid, execution_id, auth)
        return await self.block_state.get_block_result(user_uuid, execution_id)

    @handle("/block/state/set")
    async def set_block_state(self, auth, user_uuid, execution_id, data):
        self._validate_key(user_uuid, execution_id, auth)
        await self.block_state.set_block_result(user_uuid, execution_id, data)

    @handle("/block/state/delete")
    async def delete_block_state(self, auth, user_uuid, execution_id):
        self._validate_key(user_uuid, execution_id, auth)
        await self.block_state.delete_block_result(user_uuid, execution_id)
