...
def get_python38_lambda_base_zip(aws_client_factory, credentials, libraries):
    ...
    build_id = start_python38_codebuild(
        aws_client_factory,
        credentials,
        libraries_object
    )
...
def start_python38_codebuild(aws_client_factory, credentials, libraries_object):
    """
    ...
    codebuild_response = codebuild_client.start_build(
        ...
        imageOverride="docker.io/python:3.8",
...
def get_python38_base_code(app_config, code):
    code = code + "\n\n" + app_config.get("LAMDBA_BASE_CODES")["python3.8"]
    return code


def build_python38_lambda(app_config, aws_client_factory, credentials, code, libraries):
    code = get_python38_base_code(
        app_config,
        code
    )
    ...
    if len(libraries) > 0:
        base_zip_data = get_python38_lambda_base_zip(
            aws_client_factory,
            credentials,
            libraries
        )
