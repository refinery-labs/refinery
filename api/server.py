...
from utils.deployments.awslambda import LambdaManager
from utils.deployments.schedule_trigger import ScheduleTriggerManager
from utils.deployments.sns import SNSManager
from utils.deployments.sqs import SQSManager
from utils.deployments.api_gateway import strip_api_gateway, APIGatewayManager
...
"Statement": [
{
  ...
  "Action": "sts:AssumeRole"
	...
	@staticmethod
	def _create_new_sub_aws_account( db_session_maker, aws_organization_client, account_type, aws_account_id ):
	...
	@staticmethod
	def _get_sub_account_billing_data( db_session_maker, aws_cost_explorer, account_id, account_type, start_date, end_date, granularity, use_cache ):
		"""
	...
	@staticmethod
	def _get_sub_account_service_breakdown_list( db_session_maker, aws_cost_explorer, account_id, account_type, start_date, end_date, granularity, use_cache ):
		"""
		Return format:
	...
	@staticmethod
	def _api_get_sub_account_billing_data( db_session_maker, aws_cost_explorer, account_id, account_type, start_date, end_date, granularity ):
		"""
	...
	@staticmethod
	def _delete_cached_inline_execution_lambda( db_session_maker, lambda_manager, credentials, arn, lambda_uuid ):
	...
	@staticmethod
	def _cache_inline_lambda_execution( db_session_maker, lambda_manager, credentials, language, timeout, memory, environment_variables, layers, libraries, arn, lambda_size ):
...
def clear_sub_account_packages( credentials ):
	while True:
		package_paths = yield self.local_tasks.get_build_packages(
			credentials,
			"",
			1000
		)
		...
		yield self.local_tasks.bulk_s3_delete(
			credentials,
			credentials[ "lambda_packages_bucket" ],
			package_paths
		)
