		...
		def generate_managed_accounts_invoices( self, start_date_string, end_date_string ):
			...
			for organization_id in organization_ids:
				...
				for aws_account_dict in organization_aws_accounts:
					...
					print( "Account: " + aws_account_dict[ "account_id" ] + " for " + start_date_string + " -> " + end_date_string )
...
def deploy_lambda( credentials, id, name, language, code, libraries, max_execution_time, memory, transitions, execution_mode, execution_pipeline_id, execution_log_level, environment_variables, layers, reserved_concurrency_count ):
	...
	* Update the source code with the new layer ARN
	...
	if language == "nodejs8.10":
		layers.append(
			"arn:aws:lambda:us-west-2:134071937287:layer:refinery-node810-custom-runtime:14"
		)
	elif language == "php7.3":
		layers.append(
			"arn:aws:lambda:us-west-2:134071937287:layer:refinery-php73-custom-runtime:14"
		)
	elif language == "go1.12":
		layers.append(
			"arn:aws:lambda:us-west-2:134071937287:layer:refinery-go112-custom-runtime:14"
		)
