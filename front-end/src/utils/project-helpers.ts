...
export function getValidTransitionsForNode(project: RefineryProject, fromNode: WorkflowState): AvailableTransitionsByType {
...
export function getValidBlockToBlockTransitions(state: ProjectViewState) {
  if (!state.openedProject || !state.selectedResource
    || !state.isAddingTransitionCurrently || !state.availableTransitions) {
    return null;
  }
