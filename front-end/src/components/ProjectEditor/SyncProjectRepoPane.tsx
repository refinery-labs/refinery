...
import { isDevelopment, RepoManagerStoreModule } from '@/store';
...
export default class SyncProjectRepoPane extends mixins(CreateToastMixin) {
  ...
  public async showGitPushErrors() {
    if (!RepoManagerStoreModule.getGitPushResult) {
      return;
    }

    if (RepoManagerStoreModule.getGitPushResult !== GitPushResult.Success) {
      this.displayErrorToast('Git push failure', RepoManagerStoreModule.getGitPushResultMessage);
      return;
    }
    this.displaySuccessToast('Git push success', RepoManagerStoreModule.getGitPushResultMessage);
    await RepoManagerStoreModule.clearGitPushResult();
  }
  ...
  public async diffCompiledProject() {
    ...
    if (!this.hasToggledNewBranch) {
      try {
        await RepoManagerStoreModule.diffCompiledProjectAndRemoveBranch();
      } catch (e) {
        this.displayErrorToast('Error diffing branch', e.toString());
      }
    } else {
      await RepoManagerStoreModule.checkRemoteBranchName();
    }
  }

  get gitPushResult() {
    return RepoManagerStoreModule.gitPushResult;
  }
  get remoteBranchName() {
    return RepoManagerStoreModule.remoteBranchName;
  }

  public changeCurrentlyDiffedFile(file: string) {
    RepoManagerStoreModule.setCurrentlyDiffedFile(file);
  }
  ...
  public getOriginalContent(currentlyDiffedFile: string | null): string {
    ...
    const originalFileContent = RepoManagerStoreModule.gitDiffInfo.originalFiles[currentlyDiffedFile];
  ...
  public getNewContent(currentlyDiffedFile: string | null): string {
    ...
    return RepoManagerStoreModule.gitDiffInfo.changedFiles[currentlyDiffedFile];
  }
  ...
  public renderModal() {
    ...
    const diffFiles = Object.keys(RepoManagerStoreModule.gitDiffInfo.changedFiles).map(file => {
      return { value: file, text: file };
    });

    const currentlyDiffedFile = RepoManagerStoreModule.getCurrentlyDiffedFile;
  ...
  public renderGitStatusDetails() {
    const stats = RepoManagerStoreModule.getGitStatusStats;
    return (
  ...
  public renderGitStatus() {
    if (!RepoManagerStoreModule.creatingNewBranch && RepoManagerStoreModule.gitStatusResult.length > 0) {
      return (
        <div>
          <hr />
  ...
  public async forcePushToRepo() {
    await RepoManagerStoreModule.forcePushToRemoteBranch();
  ...
  public renderForcePushWarning() {
    ...
    return (
      <b-modal
        on={modalOnHandlers}
        hide-footer={true}
        title={`Force push to the branch ${RepoManagerStoreModule.remoteBranchName}?`}
      ...
      >
        <b-form on={{ submit: preventDefaultWrapper(this.forcePushToRepo) }}>
          ...
          <p class="text-bold text-align--center">{RepoManagerStoreModule.remoteBranchName}</p>
          <p>
          ...
          <div class="display--flex">
            <b-button
              ...
              variant="danger"
              type="submit"
              disabled={RepoManagerStoreModule.isPushingToRepo}
            >
              Confirm Force Push
            </b-button>
          </div>
  ...
  public renderCommitButtons() {
    if (RepoManagerStoreModule.getGitPushResult === GitPushResult.UnableToFastForward) {
      return (
        <div class="mt-2">
          <b-button
            variant="danger"
    ...
    return (
      <div class="mt-2">
        <b-button
          ...
          class="col-12"
          type="submit"
          disabled={RepoManagerStoreModule.isPushingToRepo || RepoManagerStoreModule.isDiffingBranch}
          on={{ click: RepoManagerStoreModule.pushToRemoteBranch }}
        >
          Push to branch
        </b-button>
      </div>
  ...
  public setRemoteBranchName(branchName: string) {
    if (RepoManagerStoreModule.creatingNewBranch) {
      return;
    }

    RepoManagerStoreModule.setRemoteBranchName(branchName);
    RepoManagerStoreModule.clearGitStatusResult();
  }

  public setNewRemoteBranchName(branchName: string) {
    RepoManagerStoreModule.setNewRemoteBranchName(branchName);
    RepoManagerStoreModule.clearGitStatusResult();
  }

  public async runGitShellCommand(command: string) {
    this.gitCommandResult = await RepoManagerStoreModule.runGitCommand(command);
  }

  public setCreatingNewBranch(creatingNewBranch: boolean) {
    if (creatingNewBranch && !this.hasToggledNewBranch) {
      RepoManagerStoreModule.setRandomRemoteBranchName();
      this.hasToggledNewBranch = true;
    }
    if (!creatingNewBranch) {
      this.hasToggledNewBranch = false;
    }
    RepoManagerStoreModule.setCreatingNewBranch(creatingNewBranch);
  }

  public getFirstBranchName() {
    const repoBranches = RepoManagerStoreModule.repoBranches;

    if (repoBranches.includes(RepoManagerStoreModule.remoteBranchName)) {
      return RepoManagerStoreModule.remoteBranchName;
    }
  ...
  public showDevelopmentGitShell() {
    return (
      <div>
        <h4>Git Shell</h4>
        <p>Project Session ID: {RepoManagerStoreModule.projectSessionId}</p>
  ...
  public renderSelectExistingBranchCard() {
    ...
    return (
      <b-card
        no-body
        bg-variant={!RepoManagerStoreModule.creatingNewBranch ? 'light' : 'default'}
      ...
      >
        ...
        <b-collapse
          ...
          visible={!RepoManagerStoreModule.creatingNewBranch}
  ...
  public renderCreateNewBranchCard() {
    const remoteBranchName = RepoManagerStoreModule.getRemoteBranchName;

    const repoBranches = RepoManagerStoreModule.repoBranches;
    const visible = repoBranches.length === 0 || RepoManagerStoreModule.creatingNewBranch;
        ...
        <b-collapse id="new-branch-collapse" visible={visible} accordion="repo-branch-accordion" role="tabpanel">
          <b-card-body>
            <b-form-input
              ...
              autofocus={true}
              required={true}
              state={RepoManagerStoreModule.isValidRemoteBranchName}
              value={remoteBranchName}
              on={{ input: this.setNewRemoteBranchName }}
              placeholder="eg, new-feature"
            />
            <b-form-invalid-feedback className="padding--small" state={RepoManagerStoreModule.isValidRemoteBranchName}>
  ...
  public renderRepoBranchSelect() {
    const gitBranchProps: LoadingContainerProps = {
      show: RepoManagerStoreModule.isDiffingBranch,
  ...
  public render(h: CreateElement): VNode {
    const loadingClasses = {
      'whirl standard': !RepoManagerStoreModule.gitStatusResult,
    ...
    const gitPushProps: LoadingContainerProps = {
      show: RepoManagerStoreModule.isPushingToRepo,
      ...
      <div class={loadingClasses}>
        <Loading props={gitPushProps}>
          ...
          <b-form-input
            type="text"
            required={true}
            value={RepoManagerStoreModule.commitMessage}
            on={{ input: RepoManagerStoreModule.setCommitMessage }}
            placeholder=""
          />
