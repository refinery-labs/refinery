...
import {
  ...
  transitionTypeToConfigLookup, availableTransitions
...
export default class AddTransitionPane extends Vue {
  ...
  public renderTransitionSelect(key: WorkflowRelationshipType, transition: AddGraphElementConfig | null) {
    ...
    return (
      <b-list-group-item on={{click: () => this.selectTransitionTypeToAdd(key)}}
                         disabled={!isTransitionEnabled}
                         active={this.newTransitionTypeSpecifiedInAddFlow === key}
                         class={groupItemClasses} button>
  ...
  renderHelpText() {
    return (
      <b-list-group class="add-transition-container" style={{margin: '0 0 2px 0'}}>
        <div >
          <b-list-group horizontal>
            {availableTransitions.map(
              (key) => this.renderTransitionSelect(key, transitionTypeToConfigLookup[key])
            )}
          </b-list-group>
        </div>
        ...
        <b-list-group-item>
          <b-button variant="danger" class="col-md-12"
